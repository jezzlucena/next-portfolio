    ...
    >
      <div className={styles.wrapper} ref={wrapper}>
        ...
        <div className={styles.detailsContainer}>
          <div className={`${styles.name} ${styles.loadingGradient}`}>{t(project.i18nKeys.name)}</div>
          <div className={`${styles.subtitle} ${styles.loadingGradient}`}>
            <span className={styles.role}>{t(project.i18nKeys.role)}</span>
            <span className={styles.company}>{t(project.i18nKeys.company)}</span>
            <span className={styles.year}>{project.year}</span>
          </div>
          <div className={`${styles.description} ${styles.loadingGradient}`}>{t(project.i18nKeys.description)}</div>
        </div>
      </div>
    </Link>
  );
